...
VIEWS.pengguna = () => {
  ...
  if (T === 'users') {
    ...
    body = `<div class="row between"><div class="muted">200 aktivitas terakhir. <b>Server</b> = diverifikasi server (masuk, gagal masuk, perubahan akun). <b>Aplikasi</b> = dilaporkan modul di browser (void, batal bill).</div><button class="btn btn-sm" data-act="admin-reload">${icon('refresh-cw', 14)} Muat ulang</button></div>
      <div class="card"><div class="table-wrap"><table class="tbl"><thead><tr><th>Waktu</th><th>Pengguna</th><th>Aktivitas</th><th>Detail</th><th>Sumber</th><th>IP</th></tr></thead><tbody>
      ${ADMIN.logs.map(a => `<tr><td class="mono" style="color:var(--ink-500)">${fmtDT(a.t)}</td><td class="strong">${esc(a.who)}</td><td>${/Gagal|ditolak|terkunci/i.test(a.event) ? `<span class="pill bad">${esc(a.event)}</span>` : /Masuk/.test(a.event) ? `<span class="pill ok">${esc(a.event)}</span>` : `<span class="pill info">${esc(a.event)}</span>`}</td><td class="muted">${esc(a.detail)}</td><td>${a.source === 'client' ? '<span class="pill">Aplikasi</span>' : '<span class="pill info">Server</span>'}</td><td class="mono" style="color:var(--ink-500)">${esc(a.ip)}</td></tr>`).join('') || `<tr><td colspan="6">${emptyState('history', 'Belum ada aktivitas.')}</td></tr>`}
      </tbody></table></div></div>`;
  }
